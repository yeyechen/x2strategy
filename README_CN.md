...
> *“请分析这篇论文并实现其中的主要策略。”* + 附上一篇 PDF 论文 + 可选的数据文件 + 可选的说明／用户自定义提示，例如：*“当论文中对某个公式描述不明确时，你可以直接向我提问，或者参考 sample_instruction.md 文件。”*
...
默认生成文件应写入 `PAPER2SPEC_REPLICATIONS_PATH/<slug>/`，例如 `content.json`、`spec.json`、生成的实现文件以及 `results/metrics.json`。如果引用了已有的 Copilot、VS Code 或其他 Agent 日志路径，但该路径为空或缺少必要文件，应从原始论文、说明文件和数据重新生成，而不是依赖日志摘要。
...
> [!IMPORTANT]
> 不是所有由 skill 提取出的策略都适合直接连接 broker 做 live/paper trading。很多研究策略的输入本身就是因子收益、合成组合、排序面板或其他不可直接交易的数据；也有一些策略本质上是组合构造、SDF 或资产定价流程，而不是实时下单规则。开源 skill 侧重有依据的提取、代码生成、验证和研究回测。若策略和数据契约适合实盘/模拟盘，ALAGENT 网站可以生成连接 broker 的策略。
...
│   ├── config.py               #   环境变量与 replications 路径配置
...
| 环境变量 | 默认值 | 说明 |
|:---------|:-------|:-----|
| `PAPER2SPEC_REPLICATIONS_PATH` | `./replications` | 输出根目录 |
